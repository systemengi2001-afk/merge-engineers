...
export default function AffiliateHome() {
  ...
  return (
    <main className={styles.page}><div className={styles.shell}>
      ...
      {featured && <section className={styles.featured} aria-label="最優先のおすすめ"><div className={styles.featuredLabel}>EDITOR&apos;S PICK / まず見るならこれ</div><div className={styles.featuredGrid}><div><p className={styles.featuredKicker}>販売導線を1つにまとめたい人向け</p><h2>LP、メール、講座、販売導線。<br />バラバラに契約する前に。</h2><p>systeme.ioは、複数ツールをまたぐ手間を減らしたい個人事業主・副業ユーザーの有力候補です。無料プランはカード不要。まず触ってから、必要になった時だけ有料化できます。継続利用を決めた人は、年払いなら公式価格で2か月分安くなるため月払いとの比較も先に確認してください。</p></div><div className={styles.featuredAction}><a href={featured.affiliateUrl} target="_blank" rel="nofollow sponsored noopener noreferrer">カード不要で無料プランを試す ↗</a><Link href="/affiliate/systeme-io-annual-plan/">年払いなら2か月分お得｜3プランを比較 →</Link><Link href="/affiliate/systeme-io-pricing/">無料・月払い・年払いの選び方 →</Link><small>※ 最初のボタンは承認済みアフィリエイトリンクです。料金・仕様は契約前に公式画面をご確認ください。</small></div></div></section>}
